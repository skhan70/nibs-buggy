...
function getItems(req, res, next) {
    var userId = req.userId;
    db.query("SELECT id, name, description, nibs_image__c AS image, productPage__c AS productPage, publishDate__c AS publishDate FROM wishlist, salesforce.product2 WHERE productId = id AND userId=$1 ORDER BY publishDate DESC LIMIT $2",
            [userId, 20])
